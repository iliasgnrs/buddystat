...
description: Configure Laravel routes and middleware to proxy BuddyStat tracking
---
...
Laravel makes it straightforward to proxy BuddyStat tracking using routes and the HTTP client. This guide shows how to set up proxy endpoints in your Laravel application.

## Overview

Laravel's HTTP client (built on Guzzle) provides an elegant way to proxy requests to BuddyStat servers while maintaining full control over headers, caching, and error handling.

**What you'll achieve:**
- Proxy all BuddyStat endpoints through your Laravel app
...
Add your BuddyStat host to `.env`:

```bash
# .env
RYBBIT_HOST=https://app.buddystat.com
...
class AnalyticsProxyController extends Controller
{
    ...
    {
        $this->rybbitHost = config('services.rybbit.host', 'https://app.buddystat.com');
    }
    ...
    /**
     * Forward request to BuddyStat backend
     */
    private function forwardRequest(string $path, string $method, Request $request)
    {
        ...
        try {
            ...
            \Log::error('BuddyStat proxy error', [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
...
Add BuddyStat configuration to `config/services.php`:
...
return [
    ...
    'rybbit' => [
        'host' => env('RYBBIT_HOST', 'https://app.buddystat.com'),
    ],
];
```
...
4. **Verify in BuddyStat dashboard**: Data should appear

</Steps>

## How It Works

Laravel routes intercept requests to `/analytics/*` and forward them to BuddyStat:

1. Request to `/analytics/script.js` hits Laravel route
2. Controller forwards to `https://app.buddystat.com/api/script.js`
...
**Problem**: Old script cached after BuddyStat update.
