...
description: Configure Caddy reverse proxy to serve BuddyStat tracking with automatic HTTPS
---
...
Caddy is a powerful web server with automatic HTTPS that makes reverse proxying simple and secure. This guide shows how to configure Caddy to proxy BuddyStat tracking through your own domain.
...
yourdomain.com {
    ...
    handle /analytics/script.js {
        reverse_proxy https://app.buddystat.com {
            header_up Host app.buddystat.com
...
This proxies all `/analytics/*` requests to `https://app.buddystat.com/api/*`.
...
3. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in BuddyStat dashboard

</Steps>

## How It Works

Caddy's `reverse_proxy` directive forwards requests to BuddyStat's servers:
...
4. Forwards to `https://app.buddystat.com/api/script.js` with proper headers
5. Returns response to browser

The BuddyStat script auto-detects it's being served from your domain and sends all tracking to your domain's endpoints.
...
{$DOMAIN:yourdomain.com} {
    handle_path /analytics/* {
        reverse_proxy {$RYBBIT_HOST:https://app.buddystat.com} {
...
export RYBBIT_HOST=https://app.buddystat.com
caddy run
```
...
**Solution**:
1. Verify BuddyStat backend is accessible:
   ```bash
...
**Problem**: Requests go to wrong BuddyStat endpoints.
