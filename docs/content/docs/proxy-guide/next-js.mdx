...
description: Configure Next.js rewrites to proxy BuddyStat tracking and bypass ad blockers
---
...
Next.js provides a powerful [rewrites](https://nextjs.org/docs/app/api-reference/next-config-js/rewrites) feature that makes it easy to proxy BuddyStat tracking through your own domain. This guide works with both the App Router (Next.js 13+) and Pages Router.

## Overview

Next.js rewrites allow you to map incoming request paths to different destination URLs transparently. This is perfect for proxying analytics since the browser sees requests to your domain while Next.js forwards them to BuddyStat's servers behind the scenes.

**What you'll achieve:**
- Serve BuddyStat scripts from your domain (e.g., `/analytics/script.js`)
...
Create or update your `.env` or `.env.local` file with your BuddyStat instance URL:
...
```bash
# .env.local
NEXT_PUBLIC_RYBBIT_HOST=https://app.buddystat.com
```
...
Replace `analytics.yourcompany.com` with your actual BuddyStat instance domain.
...
This configuration proxies all BuddyStat endpoints including session replay and Web Vitals:
...
const nextConfig = {
  async rewrites() {
    ...
    if (!rybbitHost) {
      console.warn('NEXT_PUBLIC_RYBBIT_HOST is not set. BuddyStat proxy will not work.');
      return [];
    }
...
Update your script tag to load from the proxied path instead of directly from BuddyStat:
...
2. **Verify the script loads**:
   - You should see a request to `/analytics/script.js` (not `app.buddystat.com`)
...
3. **Verify tracking works**:
   ...
   - All requests should show your domain, not `app.buddystat.com`
4. **Check the BuddyStat dashboard**:
...
The BuddyStat tracking script has a built-in auto-discovery mechanism. When it loads from:
...
This means all tracking requests automatically go to `/analytics/track`, `/analytics/identify`, etc., which Next.js then rewrites to forward to BuddyStat's servers.
...
The BuddyStat server sets appropriate cache headers, which Next.js respects:
...
4. Ensure your BuddyStat instance URL is correct in `.env.local`
