...
description: Common issues and solutions for BuddyStat proxy configurations
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Steps } from 'fumadocs-ui/components/steps';

This guide covers common issues you might encounter when setting up a BuddyStat proxy and how to resolve them.
...
Verify BuddyStat backend is accessible:
...
- No tracking data appears in BuddyStat dashboard
...
Tracking endpoints need proper CORS headers. BuddyStat sets these automatically, but ensure your proxy preserves them:
...
// ✅ Correct - allow all origins (or use BuddyStat's headers)
...
proxy_pass https://app.buddystat.com;
```
...
**Symptoms:**
- Changed settings in BuddyStat dashboard
...
```bash
# Test latency to BuddyStat
curl -w "@curl-format.txt" -o /dev/null -s https://app.buddystat.com/api/script.js
...
- Self-hosting BuddyStat closer to your users
- Using a CDN proxy
...
```bash
curl -v https://app.buddystat.com/api/script.js
```
...
```bash
git add vercel.json
git commit -m "Add BuddyStat proxy config"
git push
```
...
5. **Verify directly** against `https://app.buddystat.com/api/script.js`
