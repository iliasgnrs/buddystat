...
description: Framework-agnostic guide to proxy BuddyStat tracking through your own domain
---
...
This guide covers the core concepts for setting up a BuddyStat proxy with any reverse proxy or server framework. If your specific framework has a dedicated guide, we recommend following that instead for optimized instructions.
...
- **Your BuddyStat instance URL**:
  - Cloud: `https://app.buddystat.com`
  - Self-hosted: Your BuddyStat instance URL (e.g., `https://analytics.yourcompany.com`)
- **Your BuddyStat site ID** (found in your BuddyStat dashboard)

## Understanding the Endpoints

BuddyStat uses several endpoints for tracking. You can choose between a minimal setup or full proxy:
...
This setup supports all BuddyStat features including session replay, Web Vitals, and custom events.
...
Set up your reverse proxy to forward requests from your chosen path (e.g., `/analytics/*`) to the BuddyStat server.

**Generic proxy configuration pattern:**

```
Your domain path          →  BuddyStat backend
-----------------            ---------------
/analytics/script.js      →  https://app.buddystat.com/api/script.js
/analytics/track          →  https://app.buddystat.com/api/track
/analytics/replay.js      →  https://app.buddystat.com/api/replay.js
... and so on
```
...
Implement caching to reduce load on BuddyStat servers and improve performance:
...
Change your script tag to load from your proxied domain instead of directly from BuddyStat:

**Before (direct loading):**
```html
<script src="https://app.buddystat.com/api/script.js" async data-site-id="YOUR_SITE_ID"></script>
```
...
<Callout type="info">
**How Auto-Discovery Works**: The BuddyStat script automatically extracts the analytics host from its own `src` attribute by splitting on `/script.js`. This means when it loads from `https://yourdomain.com/analytics/script.js`, it automatically sends all tracking data to `https://yourdomain.com/analytics/*` endpoints.
</Callout>
...
5. **Check your BuddyStat dashboard**:
...
**CORS headers** (should be preserved from BuddyStat):
```
...
For accurate tracking, ensure these headers are forwarded to the BuddyStat backend:
...
<Callout type="warn">
**IP Forwarding**: BuddyStat uses IP addresses for geolocation (country/city) and creating anonymous user identifiers (hashed IP + User Agent). If you don't forward the `X-Forwarded-For` or `X-Real-IP` header, all traffic will appear to come from your proxy server's IP.
</Callout>
...
Configure your proxy to maintain persistent connections to BuddyStat's backend to reduce latency on repeated requests.
...
```
https://analytics.yourdomain.com/script.js  →  https://app.buddystat.com/api/script.js
https://analytics.yourdomain.com/track      →  https://app.buddystat.com/api/track
```
...
```
https://yourdomain.com/analytics/script.js  →  https://app.buddystat.com/api/script.js
https://yourdomain.com/analytics/track      →  https://app.buddystat.com/api/track
```
...
- Whitelist your BuddyStat backend IP if possible
...
2. **CORS errors**: Ensure CORS headers are preserved from BuddyStat backend
