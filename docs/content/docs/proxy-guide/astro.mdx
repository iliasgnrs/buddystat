...
description: Configure Astro middleware to proxy BuddyStat tracking requests
---

import { Steps } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

Astro's middleware feature makes it simple to proxy BuddyStat tracking. This guide shows how to set up server-side request proxying in your Astro application.
...
**What you'll achieve:**
- Proxy BuddyStat endpoints through your Astro site
...
const RYBBIT_HOST = import.meta.env.RYBBIT_HOST || 'https://app.buddystat.com';
...
async function proxyToRybbit(request: Request, pathname: string): Promise<Response> {
  try {
    ...
    console.error('BuddyStat proxy error:', error);
...
3. **Verify in BuddyStat dashboard**: Data should appear
...
2. Middleware proxies to `https://app.buddystat.com/api/script.js`
...
async function proxyToRybbit(request: Request, pathname: string): Promise<Response> {
  try {
    ...
    console.error('BuddyStat proxy error:', errorDetails);
