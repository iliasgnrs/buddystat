...
description: Configure Express.js middleware to proxy BuddyStat tracking requests
---
...
Express.js makes it easy to proxy BuddyStat tracking using middleware. This guide shows how to set up a reverse proxy using `http-proxy-middleware`.

## Overview

The `http-proxy-middleware` package provides a simple way to proxy requests in Express applications, making it perfect for routing analytics requests to BuddyStat servers.

**What you'll achieve:**
- Proxy all BuddyStat endpoints through your Express app
...
const app = express();
const RYBBIT_HOST = process.env.RYBBIT_HOST || 'https://app.buddystat.com';

// BuddyStat Analytics Proxy
// Proxy all /analytics/* requests to BuddyStat
...
The `http-proxy-middleware` intercepts requests matching the specified paths and forwards them to BuddyStat:

1. Request to `/analytics/script.js` is intercepted
2. Path is rewritten to `/api/script.js`
3. Request is forwarded to `https://app.buddystat.com/api/script.js`
...
**Problem**: 404 errors on BuddyStat endpoints.
