...
description: Configure Nginx reverse proxy to serve BuddyStat tracking and bypass ad blockers
---
...
Nginx is a powerful web server and reverse proxy that makes it easy to proxy BuddyStat tracking through your own domain. This guide shows how to configure Nginx to forward tracking requests to BuddyStat servers while caching static scripts for better performance.

## Overview

Nginx's `proxy_pass` directive lets you forward requests from your domain to BuddyStat's servers. Combined with caching, this provides excellent performance and reliability for proxying analytics.

**What you'll achieve:**
- Proxy all BuddyStat endpoints through your domain
- Cache static scripts for better performance
- Forward necessary headers for accurate tracking
- Support all BuddyStat features (session replay, Web Vitals, custom events)
...
- Your BuddyStat instance URL:
  - **Cloud hosted**: `https://app.buddystat.com`
  - **Self-hosted**: Your instance URL
- Your BuddyStat site ID (found in your dashboard)
...
server {
    ...
    # BuddyStat Analytics Proxy
    # Main tracking script (cached)
    location = /analytics/script.js {
        proxy_pass https://app.buddystat.com/api/script.js;
        proxy_set_header Host app.buddystat.com;
    ...
    location = /analytics/replay.js {
        proxy_pass https://app.buddystat.com/api/replay.js;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/metrics.js {
        proxy_pass https://app.buddystat.com/api/metrics.js;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/track {
        proxy_pass https://app.buddystat.com/api/track;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/identify {
        proxy_pass https://app.buddystat.com/api/identify;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/session-replay/record/(.*)$ {
        proxy_pass https://app.buddystat.com/api/session-replay/record/$1;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/site/tracking-config/(.*)$ {
        proxy_pass https://app.buddystat.com/api/site/tracking-config/$1;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
<Callout type="info">
Replace `app.buddystat.com` with your self-hosted BuddyStat instance URL if you're not using the cloud version.
</Callout>
...
http {
    # Define cache zone for BuddyStat
...
Replace `YOUR_SITE_ID` with your actual site ID from the BuddyStat dashboard.
...
4. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in your BuddyStat dashboard after 1-2 minutes

</Steps>

## How It Works

Nginx's `proxy_pass` directive transparently forwards requests to BuddyStat's servers:

1. Browser requests `https://yourdomain.com/analytics/script.js`
2. Nginx matches the location block and forwards to `https://app.buddystat.com/api/script.js`
...
The BuddyStat script auto-detects it's being served from your domain and sends all tracking data to your domain's `/analytics/*` endpoints.
...
location ~ ^/analytics/(script|replay|metrics)\.js$ {
    proxy_pass https://app.buddystat.com/api/$1.js;
    # ... proxy headers
    proxy_cache_valid 200 1h;
...
location ~ ^/analytics/(track|identify)$ {
    proxy_pass https://app.buddystat.com/api/$1;
    # ... proxy headers
...
Keep connections to BuddyStat backend alive for better performance:

```nginx
upstream rybbit_backend {
    server app.buddystat.com:443;
    keepalive 32;
}
...
```nginx
location ~ ^/analytics/session-replay/record/ {
    proxy_pass https://app.buddystat.com/api/session-replay/record/;
    # ... proxy headers
...
**Solution**:
1. Check BuddyStat backend is accessible:
   ```bash
...
### Incorrect geolocation in BuddyStat
...
**Problem**: Logs show SSL verification errors when connecting to BuddyStat.

**Solution**:
Add `proxy_ssl_server_name on` to enable SNI:
```nginx
location /analytics/ {
    proxy_pass https://app.buddystat.com/api/;
...
server {
    ...
    # Proxy all requests to BuddyStat
    location / {
        proxy_pass https://app.buddystat.com/api/;
        proxy_set_header Host app.buddystat.com;
        proxy_set_header X-Real-IP $remote_addr;
...
### Multiple BuddyStat Instances

If you need to proxy multiple BuddyStat instances:

```nginx
location /analytics-prod/ {
    proxy_pass https://prod.buddystat.com/api/;
    # ... proxy config
}

location /analytics-staging/ {
    proxy_pass https://staging.buddystat.com/api/;
    # ... proxy config
}
```
...
- [Nginx Custom Configuration Guide](/docs/self-hosting-guides/custom-nginx) - Self-hosting BuddyStat with Nginx
