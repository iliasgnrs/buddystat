...
description: Use Cloudflare Workers to proxy BuddyStat tracking at the edge
---
...
Cloudflare Workers provide a serverless way to proxy BuddyStat tracking at the edge, close to your users worldwide. This delivers excellent performance with minimal configuration.
...
```javascript
// Cloudflare Worker for BuddyStat Analytics Proxy

const RYBBIT_HOST = 'https://app.buddystat.com';
...
```javascript
// Minimal Cloudflare Worker for BuddyStat
...
async function handleRequest(request, env) {
  const RYBBIT_HOST = env.RYBBIT_HOST || 'https://app.buddystat.com';
  const url = new URL(request.url);
...
3. If not cached, proxies to `app.buddystat.com/api/script.js`
