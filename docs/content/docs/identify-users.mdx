...
description: How to identify and track logged-in users with BuddyStat
---
...
BuddyStat allows you to identify logged-in users and associate their activity with a persistent identity across devices and sessions. This enables powerful user-level analytics, including tracking user journeys, understanding retention, and personalizing experiences.

![identify users](/docs/users.png)

## How It Works

By default, BuddyStat assigns an anonymous ID to each visitor based on their device fingerprint. When you call `identify()`, you link this anonymous visitor to a known user ID, allowing you to:
...
When identifying users, certain trait fields are treated specially by BuddyStat:
...
- `traits` (Object, Optional): An object containing user metadata like email, name, username, or any custom fields. This data is stored in BuddyStat and can be used to identify users in the dashboard. Setting a trait to `null` removes it from the user's profile.
