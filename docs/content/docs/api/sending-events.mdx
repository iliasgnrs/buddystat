...
1. Navigate to **Settings → Account** in your BuddyStat dashboard
...
<Tab value="Pageview">
```bash
curl -X POST 'https://app.buddystat.com/api/track' \
...
const trackEvent = async (eventData) => {
  const response = await fetch('https://app.buddystat.com/api/track', {
    method: 'POST',
    headers: {
...
def track_event(event_data):
    response = requests.post(
        'https://app.buddystat.com/api/track',
        headers={
            'Content-Type': 'application/json',
...
public class BuddyStatTracker {
    private static final String API_URL = "https://app.buddystat.com/api/track";
    ...
    public static void main(String[] args) throws Exception {
        BuddyStatTracker tracker = new BuddyStatTracker();
...
class BuddyStatTracker {
    private $apiUrl = 'https://app.buddystat.com/api/track';
    private $apiKey;
    private $siteId;
...
class BuddyStatTracker
  API_URL = 'https://app.buddystat.com/api/track'
...
public class BuddyStatTracker
{
    ...
    private const string ApiUrl = "https://app.buddystat.com/api/track";

    public BuddyStatTracker()
    {
    ...
    {
        var tracker = new BuddyStatTracker();
...
impl BuddyStatTracker {
    fn new() -> Self {
        Self {
            ...
            api_url: "https://app.buddystat.com/api/track".to_string(),
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let tracker = BuddyStatTracker::new();
...
class BuddyStatTracker {
    static let shared = BuddyStatTracker()

    private let apiUrl = "https://app.buddystat.com/api/track"
    ...
    func trackEvent(_ eventData: [String: Any]) async throws {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(domain: "BuddyStatTracker", code: 1, userInfo: [NSLocalizedDescriptionKey: "Tracking failed"])
        }
    }
...
Task {
    // Track a pageview
    try await BuddyStatTracker.shared.trackEvent([
        "type": "pageview",
        "pathname": "/api/users",
        "hostname": "api.example.com",
        "page_title": "User API Endpoint"
...
class BuddyStatTracker(private val context: Context) {
    companion object {
        private const val API_URL = "https://app.buddystat.com/api/track"
...
class MainActivity : AppCompatActivity() {
    private val tracker = BuddyStatTracker(this)
...
class BuddyStatTracker {
  static final BuddyStatTracker _instance = BuddyStatTracker._internal();
  factory BuddyStatTracker() => _instance;
  BuddyStatTracker._internal();

  static const String _apiUrl = 'https://app.buddystat.com/api/track';
...
class _MyAppState extends State<MyApp> {
  final tracker = BuddyStatTracker();
...
<Callout type="info">
**Browser Tracking**: For standard browser-based tracking, use the [BuddyStat Web SDK](/docs/sdks/web) instead. The HTTP API is designed for server-side applications, mobile apps, and custom integrations.
</Callout>
