...
description: Official JavaScript SDK for BuddyStat Analytics
---
...
<Callout type="info">
The `rrweb` package is only required if you enable session replay in your BuddyStat dashboard. The SDK will work without it for all other tracking features.
</Callout>
...
```typescript
async rybbit.init(config: BuddyStatConfig): Promise<void>;
```
...
await rybbit.init({
  analyticsHost: "https://app.buddystat.com/api",
  siteId: "1",
});
```
...
| :-------------- | :------- | :----------------------------------------------------------------------------------- |
| `analyticsHost` | `string` | URL of your BuddyStat analytics instance (e.g., `https://buddystat.yourdomain.com/api`). |
| `siteId`        | `string` | The Site ID for your website obtained from your BuddyStat instance.       |
...
These settings are controlled through your BuddyStat dashboard and cannot be set in the init config:
...
The SDK can automatically capture JavaScript errors and unhandled promise rejections. This feature must be enabled in your BuddyStat dashboard.
...
Session replay must be enabled in your BuddyStat dashboard under. Once enabled, recording starts automatically when the SDK initializes.
...
console.log("Current BuddyStat User ID:", currentUserId);
```
