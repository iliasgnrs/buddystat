...
```typescript
import { BuddyStat } from "@rybbit/node";

const rybbit = new BuddyStat(config: BuddyStatConfig);
```
...
The `BuddyStat` constructor will throw an error if required configuration options (`analyticsHost`, `siteId`, `apiKey`) are missing or invalid.
</Callout>
...
| :-------------- | :----------------- | :------ | :----------------------------------------------------------------------------------------------- |
| `analyticsHost` | `string`           | -       | **Required.** URL of your BuddyStat analytics instance (e.g., `https://buddystat.yourdomain.com/api`). |
| `siteId`        | `string \| number` | -       | **Required.** The Site ID for your website obtained from your BuddyStat instance.                   |
