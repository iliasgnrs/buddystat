---
title: "Shopify"
description: "Integrate BuddyStat with your Shopify store to capture events, track conversions, and analyze user behavior."
---
...
BuddyStat can be integrated with your Shopify store to capture events, track conversions, and analyze user behavior.

## How to Add BuddyStat to Shopify

<Steps>
<Step>
### Retrieve Your Tracking Script

Navigate to your BuddyStat dashboard to obtain your code snippet.
...
To get the most out of BuddyStat with Shopify, you should track key e-commerce events. This typically involves adding small Liquid and JavaScript snippets to your theme files.
...
Shopify's checkout process is on a separate domain (`checkout.shopify.com`), and direct script injection into the checkout pages (beyond the "Order status" page) is limited for most plans. However, BuddyStat's script, if loaded on your main store, should persist session information.
...
*   **Currency Conversion**: Prices in Shopify Liquid objects are often in cents. Ensure you divide by 100 to get the decimal value for BuddyStat.
...
*   **Shopify Customer Events**: For more robust server-side or client-side event tracking, especially for `add_to_cart` and `begin_checkout` without relying on DOM manipulation, explore [Shopify Customer Events](https://shopify.dev/docs/api/customer-events). This allows you to subscribe to events like "product added to cart" or "checkout started" and send data to BuddyStat via a custom pixel or by triggering client-side JavaScript. This is a more advanced but often more reliable method.
*   **Testing**: Thoroughly test each event in your BuddyStat dashboard after implementation. Use your browser's developer console to check for JavaScript errors.