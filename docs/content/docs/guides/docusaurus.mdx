---
title: Docusaurus
description: Integrate BuddyStat Analytics with your Docusaurus v2+ website
---
...
This guide explains how to integrate BuddyStat with your Docusaurus v2+ website for analytics tracking. Docusaurus is a static site generator that builds a single-page application with React.

## How to Add BuddyStat to Your Docusaurus Site

Docusaurus provides a configuration option to add scripts or tags to the `<head>` of every page, which is the recommended way to include BuddyStat.

<Steps>
<Step>
### Retrieve Your Tracking Script Details

Navigate to your BuddyStat dashboard to obtain your **Site ID**. The script source will be `https://app.buddystat.com/api/script.js`.
</Step>
...
You can add the BuddyStat script using the `scripts` array or the `headTags` array within your site configuration. Using `scripts` is often cleaner for simple script additions.
...
export function onRouteUpdate({location, previousLocation}) {
  ...
  if (typeof window !== 'undefined' && window.rybbit && location.pathname !== previousLocation?.pathname) {
    // BuddyStat should auto-track pageviews, but if needed:
