---
title: Mintlify
description: Integrate BuddyStat Analytics with your Mintlify documentation site
---
...
This guide explains how to integrate BuddyStat with your Mintlify documentation site for analytics tracking. Mintlify uses a file-based approach for custom scripts where any JavaScript file in your content directory is automatically included on every documentation page.

## How to Add BuddyStat to Your Mintlify Docs
...
Navigate to your BuddyStat dashboard to obtain your **Site ID**. The script source will be `https://app.buddystat.com/api/script.js` (or your self-hosted URL if applicable).
</Step>
...
// BuddyStat Analytics Integration
(function() {
  // Create the script element
  const script = document.createElement('script');
  script.src = 'https://app.buddystat.com/api/script.js';
  script.async = true;
  script.defer = true;
...
Make sure to replace `YOUR_SITE_ID` with your actual Site ID from your BuddyStat dashboard.

If you're self-hosting BuddyStat, replace `https://app.buddystat.com/api/script.js` with your self-hosted URL (e.g., `https://your-domain.com/api/script.js`).
</Step>

<Step>
### Verify Installation

After deploying your Mintlify documentation site, open your BuddyStat dashboard to check if data is being received. You can also inspect your browser's network tab to confirm that `script.js` is loaded on your pages and the BuddyStat tracking requests are being sent.

Look for:
- The `script.js` file being loaded in the Network tab
- POST requests to your BuddyStat analytics endpoint (e.g., `/api/event`)
</Step>
</Steps>

<Callout type="info">
  **Page View Tracking**: Mintlify documentation sites are Single Page Applications (SPAs). BuddyStat's tracking script is designed to automatically detect route changes in SPAs and track them as page views. The default configuration should work out of the box without any additional setup.
</Callout>
...
You can customize the BuddyStat script behavior by adding data attributes to the script tag:
...
- `data-site-id` (required): Your BuddyStat site ID
...
- [BuddyStat Script Configuration](/docs/script)
