---
title: Angular
description: Integrate BuddyStat Analytics with your Angular application
---
...
This guide explains how to integrate BuddyStat with your Angular application.

## How to Add BuddyStat to Your Angular App

The recommended and most common method is to add the BuddyStat tracking script directly to your `src/index.html` file. This ensures the script is loaded early and reliably for your Angular application.
...
Open `src/index.html` and paste the BuddyStat tracking snippet just before the closing `</head>` tag or `</body>` tag. Placing it in the `<head>` is generally preferred for earlier loading.
...
After building and deploying your application, open your BuddyStat dashboard to check if data is being received. You can also inspect your browser's network tab to confirm that `script.js` is loaded.
</Step>
</Steps>
...
Once the BuddyStat script is installed and loaded, you can track custom events from any of your Angular components or services:
...
export class MyFeatureComponent {
  onFeatureClick(): void {
    if (window.rybbit && typeof window.rybbit.event === 'function') {
      ...
      console.warn('BuddyStat tracking not available.');
    }
  }
}
```
