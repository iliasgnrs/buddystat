---
title: WooCommerce
description: Integrate BuddyStat Analytics with your WooCommerce store
---
...
This guide explains how to integrate BuddyStat with your WooCommerce store (running on WordPress) to track e-commerce events like product views, add to carts, and purchases, in addition to standard page view analytics.

## Prerequisites

1.  **BuddyStat Installed on WordPress**: Ensure you have already installed the BuddyStat tracking script on your WordPress site using one of the methods described in our [WordPress integration guide](/docs/guides/wordpress). This guide focuses on adding e-commerce event tracking.
...
WooCommerce provides various action hooks that allow you to trigger JavaScript when specific e-commerce events occur. We'll use these to send data to BuddyStat.
...
*   **Script Loading Order**: Ensure the main BuddyStat tracking script (added via the WordPress integration guide) is loaded before these event-specific scripts attempt to call `window.rybbit`. Placing the main script in the `<head>` usually ensures this.
...
By implementing these tracking snippets, you can gain valuable insights into your WooCommerce store's performance using BuddyStat.