---
title: Bubble.io
description: Integrate BuddyStat Analytics with your Bubble.io application to track user behavior
---
...
This guide explains how to integrate BuddyStat with your Bubble.io application to track user behavior, page views, and custom events.

## How to Add BuddyStat to Your Bubble App
...
Navigate to your BuddyStat dashboard to obtain your code snippet. It will look like this:
...
-   Paste your BuddyStat tracking snippet into this box. This will add the script to the `<head>` section of every page in your Bubble app.
...
Open your live Bubble application and navigate through a few pages. Then, check your BuddyStat dashboard to see if data is being received. You can also inspect your browser's network tab to confirm that `script.js` is loaded.
</Step>
</Steps>

<Callout type="info">
  **Page View Tracking in Bubble Apps**: Bubble applications often behave like Single Page Applications (SPAs) for navigation between "pages" (which are often groups shown/hidden on a single actual HTML page). BuddyStat's script is designed to detect URL changes in SPAs.
  - If your Bubble app uses actual URL route changes for different views, BuddyStat should track these automatically.
  - If navigation primarily involves showing/hiding groups without URL changes, BuddyStat will only track the initial page load. For more granular tracking of "virtual" page views in such cases, you would need to use custom event tracking.
...
<Step>
### Add BuddyStat Tracking Code

-   In the "Run JavaScript" action's code block, add your BuddyStat event tracking code:
...
-   Check your BuddyStat dashboard to see if the custom event data is being received.
</Step>
</Steps>

By adding the global script and using "Run JavaScript" workflow actions, you can effectively integrate BuddyStat analytics into your Bubble.io application.