---
title: "ThriveCart"
description: "Integrate BuddyStat Analytics with your ThriveCart checkout pages"
---
...
BuddyStat can be integrated with ThriveCart to track cart views, purchases, and other conversion events on your checkout pages.

<Callout type="warn">
  ThriveCart strips out the standard `data-site-id` attribute from scripts. You must use the alternative installation method below for BuddyStat to work correctly.
</Callout>

## Adding the Tracking Script

The standard BuddyStat script won't work in ThriveCart because ThriveCart removes the `data-site-id` attribute. Instead, use this JavaScript snippet that dynamically creates the script element:
...
Replace `YOUR_SITE_ID` with your actual BuddyStat Site ID from your dashboard.
</Step>

<Step>
### Save and Test

Save your changes and visit your checkout page. Check your BuddyStat dashboard to confirm pageviews are being tracked.
</Step>
</Steps>
...
(function () {
  ...
  function ensureScriptLoaded() {
    ...
    if (document.querySelector('script[src="https://app.buddystat.com/api/script.js"]')) return;
  ...
  // If BuddyStat is already loaded, send immediately
  if (isReady()) return send();
...
- `SITE_ID`: Your BuddyStat Site ID
