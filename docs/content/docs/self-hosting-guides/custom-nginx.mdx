...
description: Set up BuddyStat with your own Nginx installation instead of using the built-in Caddy server
---
...
This guide shows you how to set up BuddyStat with your own Nginx installation instead of using the built-in Caddy server.
...
- BuddyStat running with `--no-webserver` flag

## Setup Steps

<Steps>
<Step>
### Run BuddyStat without Caddy

First, set up BuddyStat to expose ports without the built-in webserver:
...
If you need to use custom ports for BuddyStat:
...
**502 Bad Gateway**: Check if BuddyStat services are running:
```bash
docker compose ps
docker compose logs
```
