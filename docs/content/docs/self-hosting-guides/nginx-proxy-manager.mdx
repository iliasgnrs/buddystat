...
description: Set up BuddyStat with Nginx Proxy Manager without exposing ports directly to the host
---
...
This guide shows you how to set up BuddyStat with Nginx Proxy Manager without exposing ports directly to the host.
...
<Callout type="info">
This setup assumes you have NPM running and are capable of creating SSL certificates. You can also use any other reverse proxy as long as you can put it on the same Docker network as BuddyStat.
</Callout>
...
Create a new directory for your BuddyStat installation and create a `docker-compose.yml` file:
...
- Verify both NPM and BuddyStat are on the same network
...
**502 Bad Gateway**:
- Verify BuddyStat services are healthy: `docker compose ps`
